    ...
    internal class HatredFire : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 3; i = maxFore + 1)
            {
                ...
                Main.dust[dust0].position = Projectile.Center + Vector2.UnitY.RotatedByRandom(3.1415927410125732) * (float)Main.rand.NextDouble() * (float)Projectile.width / 2f;
                maxFore = i;
            }
            for (int j = 0; j < 10; j = maxFore + 1)
            {
                ...
                Main.dust[dust2].position = Projectile.Center + Vector2.UnitY.RotatedByRandom(3.1415927410125732) * (float)Main.rand.NextDouble() * (float)Projectile.width / 2f;
            ...
            for (int k = 0; k < 5; k = maxFore + 1)
            {
                ...
                Main.dust[dust3].position = Projectile.Center + Vector2.UnitX.RotatedByRandom(3.1415927410125732).RotatedBy((double)Projectile.velocity.ToRotation(), default) * (float)Projectile.width / 2f;
