...
namespace CalamityWeaponRemake.Content.Projectiles.Weapons.Rogue
{
    ...
    {
        public static SoundStyle BelCanto = new("CalamityWeaponRemake/Assets/Sounds/BelCanto") { Volume = 2.5f };
        ...
        public override void AI() {
            ...
            if (Main.netMode != NetmodeID.Server) {
                ...
                SparkParticle spark = new SparkParticle(Projectile.Center - Projectile.velocity * 8f, -Projectile.velocity * 0.1f, false, 9, 2.3f, color * 0.1f);
        ...
        public static void SpanDimensionalWave(Vector2 spanPos, Vector2 vr, Color color1, Color color2, float starS, float endS, float starS2, float endS2) {
            CWRParticle pulse = new DimensionalWave(spanPos - vr * 0.52f, vr / 1.5f, color1, new Vector2(1f, 2f), vr.ToRotation(), starS, endS, 60);
            CWRParticleHandler.SpawnParticle(pulse);
            CWRParticle pulse2 = new DimensionalWave(spanPos - vr * 0.40f, vr / 1.5f * 0.9f, color2, new Vector2(0.8f, 1.5f), vr.ToRotation(), starS2, starS2, 50);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
                ...
                if (Projectile.Calamity().stealthStrike) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        ...
                        for (int j = 0; j < 16; j++) {
                            ...
                            CWRParticle pulse = new DimensionalWave(spanPos - vr * 0.52f, vr / 1.5f, Color.Gold, new Vector2(1f, 2f), vr.ToRotation(), 0.82f * slp, 0.32f, 60);
                            CWRParticleHandler.SpawnParticle(pulse);
                            CWRParticle pulse2 = new DimensionalWave(spanPos - vr * 0.40f, vr / 1.5f * 0.9f, Color.OrangeRed, new Vector2(0.8f, 1.5f), vr.ToRotation(), 0.58f * slp, 0.28f, 50);
                            CWRParticleHandler.SpawnParticle(pulse2);
                        }
                    }
            ...
            if (!Projectile.Calamity().stealthStrike) {
                CWRParticle pulse = new DimensionalWave(Projectile.Center - Projectile.velocity * 0.52f, Projectile.velocity / 1.5f, Color.Fuchsia, new Vector2(1f, 2f), Projectile.velocity.ToRotation(), 0.82f, 0.32f, 60);
                CWRParticleHandler.SpawnParticle(pulse);
                CWRParticle pulse2 = new DimensionalWave(Projectile.Center - Projectile.velocity * 0.40f, Projectile.velocity / 1.5f * 0.9f, Color.Aqua, new Vector2(0.8f, 1.5f), Projectile.velocity.ToRotation(), 0.58f, 0.28f, 50);
                CWRParticleHandler.SpawnParticle(pulse2);
            }
        ...
        public override bool PreDraw(ref Color lightColor) {
            if (PierceDrawer == null) {
                PierceDrawer = new PrimitiveTrail(WidthFunction, ColorFunction, null, GameShaders.Misc["CalamityMod:TrailStreak"]);
            }

            GameShaders.Misc["CalamityMod:TrailStreak"].SetShaderTexture(ModContent.Request<Texture2D>("CalamityMod/ExtraTextures/Trails/ScarletDevilStreak"));
            PierceDrawer.Draw(Projectile.oldPos, Projectile.Size * 0.5f - Main.screenPosition, 30);
