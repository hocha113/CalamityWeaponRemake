...
namespace CalamityWeaponRemake.Content.Projectiles.Weapons.Magic
{
    internal class GhastlyBlasts : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.numHits == 0)
            {
                ...
                if (Projectile.timeLeft < 200)
                {
                    ...
                    if (target != null)
                    {
                        ...
                        {
                            Projectile.ChasingBehavior2(target.Center);
                        }
                        else
                        {
                            Projectile.ChasingBehavior(target.Center, 16);
                        }
                    }
                }
            }
            ...
            {
                if (target != null)
                    Projectile.ChasingBehavior2(target.Center, 1, 0.5f);
            }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                int dust = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 60, 0f, 0f, 100, default, 1.5f);
            ...
            for (int j = 0; j < 30; j++)
            {
                int dustIndex = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 60, 0f, 0f, 200, default, 3.7f);
                ...
                newDust.velocity *= 3f;
                dustIndex = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 60, 0f, 0f, 100, default, 1.5f);
                newDust.position = Projectile.Center + Vector2.UnitY.RotatedByRandom(3.1415927410125732) * (float)Main.rand.NextDouble() * Projectile.width / 2f;
                newDust.velocity *= 2f;
                newDust.noGravity = true;
                newDust.fadeIn = 1f;
            ...
            for (int k = 0; k < 10; k++)
            {
                int num3 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 60, 0f, 0f, 0, default, 2.7f);
            ...
            for (int l = 0; l < 10; l++)
            {
                int num4 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 60, 0f, 0f, 0, default, 1.5f);
